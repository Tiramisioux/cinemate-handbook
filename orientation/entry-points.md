...
| A controller method / new action | A public method on `CinePiController` | **Five places**: `cli_commands.py`'s command table, `app/settings_editor.py`'s action catalogue (Python **and** its JS copy), `docs/cli-commands.md` (the old `controller-methods.md` was folded into it), and any `"method"` string under `settings.jsonc`'s `hardware_controls`/`input_peripherals` (buttons, switches, rotary encoders) | `tools/gui_field_extract.py` (gated at 0) checks the settings-editor catalogue only; `tools/docs_drift_check.py --strict`'s `methods` check separately gates `docs/cli-commands.md`; `_test/test_settings_method_names_resolve.py` gates the `"method"` strings in the **shipped** settings files — but not an operator's own `settings.jsonc` on a camera, which stays unguarded. See [`the-traps.md`](the-traps.md) #3 |
